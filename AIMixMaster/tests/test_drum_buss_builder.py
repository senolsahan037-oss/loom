...
# Committed fixtures rather than a project that was never in the repository:
# these tests could not pass on any machine but the author's. "before" is the set
# with an empty DRUM BUSS, "after" is what the real builder produced from it --
# generated by tests/make_fixture.py, never hand-copied, so the cloned ids are
# the ones the code allocates.
PROJECT = ROOT / "tests" / "fixtures" / "drum_buss_after.als"
BACKUP = ROOT / "tests" / "fixtures" / "drum_buss_before.als"
