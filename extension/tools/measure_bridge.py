...
Without an argument the MCP's own resolver names the bridge (and refuses an
old or stale one exactly as the MCP would). Writes measure_bridge.json next
to itself. Touches the open set: tempo (set and restored), one locator at
beat 320, two probe tracks, one arrangement clip at bar 81, one session
clip -- run it on a scratch set.
"""
import importlib.util, json, os, sys, time
from pathlib import Path

ROOT = Path(__file__).resolve().parents[2]  # .../Loom

# The MCP's own resolver names the bridge: no second discovery here. An
# explicit argument pins a root (a developer-mode .dev/storage/bridge).
if len(sys.argv) > 1:
    os.environ["LOOM_BRIDGE_ROOT"] = str(Path(sys.argv[1]).expanduser())
sys.path.insert(0, str(ROOT / "mcp_server"))
import bridge_client  # noqa: E402

try:
    _target = bridge_client.resolve_bridge_target()
except bridge_client.BridgeUnavailable as error:
    sys.exit(f"{error.status}: {error}")
_report = _target.protocol_report()
print(f"bridge root: {_target.root}  ({_target.source}, state age {(_target.age or 0):.1f}s, {_target.surface_version} / {_target.bridge_protocol})")
if not _report["compatible"]:
    sys.exit(f"{_report['status']}: {_report['reason']}")

os.environ["LOOM_BRIDGE_ROOT"] = str(_target.root)
