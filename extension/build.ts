...
import { SDK_API_VERSION } from "./src/bridge.js";

const manifest = JSON.parse(fs.readFileSync("manifest.json", "utf8"));
const packageJson = JSON.parse(fs.readFileSync("package.json", "utf8"));
const production = process.argv.includes("--production");
const extensionRoot = path.dirname(fileURLToPath(import.meta.url));
const senseiRoot = path.resolve(extensionRoot, "..", "Sensei");
const runtimeRoot = path.join(extensionRoot, "dist", "runtime");

// One version, written in manifest.json, checked against package.json and
// injected into the bundle (bridge.ts publishes it as surface_version). A
// mismatch is a build error, not a warning: install.py and live_bridge_status
// compare exactly these strings.
if (manifest.version !== packageJson.version) {
  throw new Error(`manifest.json version ${manifest.version} != package.json version ${packageJson.version}`);
}
if (manifest.minimumApiVersion !== SDK_API_VERSION) {
  throw new Error(`manifest.json minimumApiVersion ${manifest.minimumApiVersion} != bridge.ts SDK_API_VERSION ${SDK_API_VERSION}`);
}
...
await esbuild.build({
  ...
  minify: production,
  define: { __LOOM_EXTENSION_VERSION__: JSON.stringify(manifest.version) },
  plugins: [{
